...
export async function POST(req: Request) {
  try {
    ...
    // Properly typed message — no `as Parameters<...>` cast (that was the bug).
    ...
    const items = rawItems
      ...
      .filter((x): x is { success: true; data: z.infer<typeof itemSchema> } => x.success)
      .map((x) => x.data);

    return NextResponse.json({ items });
  } catch (err: unknown) {
    console.error("[receipt/parse] Error:", err);
    const status = (err as { status?: number }).status === 401 ? 401 : (err as { status?: number }).status === 429 ? 429 : 500;
    return NextResponse.json(
